class DataEntryway < ActiveRecord::Base
  ...
  belongs_to :provider, class_name: "DataEntrywayProvider", foreign_key: :entryway_provider
  ...
  after_update :did_logwatch

  def did_logwatch
    DIDWATCH_LOG.debug "Model says: Gateway ID #{gateway_id} is assigned to DID #{did_e164}"
  end
  ...
  def self.list_with_status_on_group(group_id)
    if group_id.present?
      ...
      return self.where(condition).select("data_entryway.id, data_entryway.title, #{status} AS status")
    end
    []
  end
  ...
  def self.get_aggregate_tracking_did(user, from_date=Date.today, to_date=Date.today, hour = 1 )
    result = self.joins("
      LEFT JOIN summary_sessions_by_entryway ON `data_entryway`.`id` = `summary_sessions_by_entryway`.`entryway_id`
      INNER JOIN `data_gateway` ON `data_gateway`.`id` = `data_entryway`.`gateway_id`")
    .select("sum(summary_sessions_by_entryway.seconds/60) as total_minutes")

    if !user.is_marketer?
      result = result.where("data_gateway.rca_id in (?)", user.data_group_rca_ids)
    end
    result.where("summary_sessions_by_entryway.hour = ? AND date BETWEEN ? and ?", hour, from_date, to_date).order("date ASC")
  end

  # dids: list of entryway ids.
  def self.get_selected_tracking_did(user, dids = [],from_date=Date.today, to_date=Date.today, hour = 1 )

    inner_sql = SummarySessionsByEntryway.where("data_entryway.id = summary_sessions_by_entryway.entryway_id").where("summary_sessions_by_entryway.hour = ? AND date BETWEEN ? and ? ", hour, from_date, to_date).select("sum(summary_sessions_by_entryway.seconds / 60) as total_minutes").to_sql
